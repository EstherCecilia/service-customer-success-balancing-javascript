...
class CustomerSuccessBalancing
    ...
    def execute
      ...
      ordered_customer_success = order_customer_success_by_score(customer_success_attending)
  
      ordered_customers = @customers.empty? ? [] : order_customers_by_score(@customers)
  
      distribute_customers_per_customer_success(ordered_customer_success, ordered_customers)
  
      customer_success_with_most_customers = group_customer_success_by_most_attendings(ordered_customer_success)
    ...
    def order_customer_success_by_score(customer_success)
      customer_success.sort_by { |cs| cs[:score] }
    end
  
    def order_customers_by_score(customers)
    ...
    def group_customer_success_by_most_attendings(customer_success)
      return [] if customer_success.empty?
  
      customer_success.group_by { |cs| cs[:customers_attending].count }.sort.reverse.first
    end
  end
